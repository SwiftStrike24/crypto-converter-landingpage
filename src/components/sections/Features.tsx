...
import ParticleWave from '../animations/ParticleWave';
...
export default function Features({
  ...
  featuresScrollYVelocity,
}: {
  heroScrollYProgress: MotionValue<number>;
  featuresScrollYProgress: MotionValue<number>;
  featuresScrollYVelocity: MotionValue<number>;
}) {
  const [activeFeature, setActiveFeature] = useState(features[0].id);

  const scale = useTransform(heroScrollYProgress, [0.3, 0.6], [0.9, 1]);
  const opacity = useTransform(heroScrollYProgress, [0.3, 0.6], [0, 1]);

  const exitScale = useTransform(featuresScrollYProgress, [0, 0.8], [1, 0.8]);
  const exitOpacity = useTransform(featuresScrollYProgress, [0, 0.5, 0.8], [1, 1, 0]);
  ...
  return (
    ...
    >
      <motion.div style={{ scale: exitScale, opacity: exitOpacity }}>
        <ParticleWave scrollYProgress={heroScrollYProgress} animationType="ambient" />
        <ParticleWave scrollYProgress={featuresScrollYProgress} scrollYVelocity={featuresScrollYVelocity} animationType="travel" />
